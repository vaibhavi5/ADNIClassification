...
def read_X_y_5D_idx(df, idx, scorename, cr, k):
    ...
    #Xc2 = np.float32((c1 - torch.mean(c1))/torch.std(c1))
    #Xc1 = np.float32((c2 - torch.mean(c2))/torch.std(c2))
    #Xc3 = np.float32((c3 - torch.mean(c3))/torch.std(c3))


    Xc1 = (Xc1 - Xc1.min()) / (Xc1.max() - Xc1.min())
    Xc2 = (Xc2 - Xc2.min()) / (Xc2.max() - Xc2.min())
    Xc3 = (Xc3 - Xc3.min()) / (Xc3.max() - Xc3.min())
